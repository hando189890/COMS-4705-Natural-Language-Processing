...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        while stack_position >= -3:
            if stack_length > 0:
                word = words[state.stack[stack_position]]
                pos_tag = pos[state.stack[stack_position]]
                if not word:
                    result[-(stack_position+1)] = self.word_vocab["<ROOT>"]
                elif word.lower() in self.word_vocab:
                    result[-(stack_position+1)] = self.word_vocab[word.lower()]
                elif pos_tag == "CD":
                    result[-(stack_position+1)] = self.word_vocab["<CD>"]
                elif pos_tag == "NNP":
                    result[-(stack_position+1)] = self.word_vocab["<NNP>"]
                else:
                    result[-(stack_position+1)] = self.word_vocab['<NULL>']
            else:
                result[-(stack_position+1)] = self.word_vocab["<NULL>"]
        ...
        while buffer_position >= -3:
            if buffer_length > 0:
                word_pos = state.buffer[buffer_position]
                pos_tag = pos[state.buffer[buffer_position]]
                if not words[word_pos]: 
                    result[-(buffer_position+1)+3] = self.word_vocab["<ROOT>"]
                elif words[word_pos].lower() in self.word_vocab:
                    result[-(buffer_position+1)+3] = self.word_vocab[words[word_pos].lower()]
                elif pos_tag == "CD":
                    result[-(buffer_position+1)+3] = self.word_vocab["<CD>"]
                elif pos_tag == "NNP":
                    result[-(buffer_position+1)+3] = self.word_vocab["<NNP>"]
                else:
                    result[-(buffer_position+1)+3] = self.word_vocab['<NULL>']
            else:
                result[-(buffer_position+1)+3] = self.word_vocab["<NULL>"]
